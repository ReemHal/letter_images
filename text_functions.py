# Class containing different methods to perform different NLP operations on a given text
import spacy
import nltk
import os
...
class text_functions_class:
    ...
    def __init__(self, text, glove_file):
        ...
        self.sent_centroids = None

        self.stop_words = set(stopwords.words('english'))
        self.glove_file = glove_file
        self.doc = text
        self.all_doc_tokens, self.doc_tokens, self.token_index = self.tokenize_doc(self.doc)
        self.voc_words, self.voc_freqs, self.total_freq = self.get_word_stats()
        self.clean_sentences, _ = self.get_sentences()
    ...
    def get_sentences(self):
            ...
            nlp = spacy.load('en')
            doc_sents=nlp(self.doc)
            sentences = [sent.string.strip() for sent in doc_sents.sents]

            return sentences, doc_sents
    ...
    def tokenize_doc(self, text):
        ...
        all_doc_tokens = nltk.word_tokenize(text)
    ...
    def get_embedding_sums(self, tokens):
        """
        Get weighted sum of embdiings in text
        """
        ...
        emb_sums = np.empty([len(tokens), len(tokens)], object)
    ...
    def get_sentence_tokens(self):
        ...
        sent_tokens = []
        for i, sent in enumerate(self.clean_sentences):
            # tokenize sent
            _, tokens, _ = self.tokenize_doc(sent)
            sent_tokens.append(tokens)

        return sent_tokens;

    def get_sent_centroids(self):

       if (self.sent_tokens == None):
           self.sent_tokens = self.get_sentence_tokens()
       if self.voc_dict is None:
           self.get_word_embed_and_entropy()

       sent_centroids = []
       for i, tokens in enumerate(self.sent_tokens):
           emb_sum = 0
           num_words = 0
           for word in tokens:
               if (word in self.voc_dict.keys()):
                   emb_sum = emb_sum +  (self.voc_dict[word]['em'] * self.voc_dict[word]['entropy'])
                   num_words += 1
           if (num_words > 0):
                sent_centroids.append(emb_sum/num_words)
           else:
                sent_centroids.append(0)

       return sent_centroids

    def summarize(self):
        '''
        returns document sentences sorted by cosine distance from the average sentence centroid
        '''
        ...
        if self.voc_dict is None:
            self.get_word_embed_and_entropy()
        if (self.sent_centroids == None):
            self.sent_centroids = self.get_sent_centroids()
        sent_distance = np.zeros(len(self.sent_centroids), dtype=np.float64)

        avg_centroid = np.average(self.sent_centroids)

        for i, centroid in enumerate(self.sent_centroids):
            sent_distance[i] = spatial.distance.cosine(centroid, avg_centroid)

        return sent_distance, np.argsort(sent_distance)
    ...
    def get_weighted_dist(self):
        ...
        if self.emb_sums is None:
            self.get_embedding_sums(self.doc_tokens)
